...
The directory contains code for the DSAA 2021 submission "Interpretable graph node embeddings by coordinated local neighborhood sampling".

The code is implemented in Python 3 and organized in several Jupyter notebooks.

The current version contains the Cora graph and the notebooks show the results of a test run on it.

In general, one has to run following steps in order to reproduce the results in the paper:
